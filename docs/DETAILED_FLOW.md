...
自律処理では復元可能な変更だけを許可する。

- 編集・置換: 変更前バックアップを作成してから実行する
- 移動・名前変更: 操作前後のパスを記録する
- 削除: 完全削除ではなくWindowsのゴミ箱への移動を基本にする
- 完全削除: 通常の自律処理では禁止し、将来も別途の明示確認を必須にする

変更操作は、対象、変更内容、バックアップ先をプレビューしてから実行する。
...
許可リスト例はGitHub、Google Calendar、Google Docsなどとするが、
認証ページ、決済ページ、秘密情報を含む画面へのアクセスは別途拒否する。
